...
**実装範囲**:
- ✅ コアバトルシステム（連打、物理シミュレーション、転倒判定、土俵判定）
- ✅ カスタム物理エンジン（重力、減衰、衝突）
- ✅ 簡易AI対戦
- ✅ UI/HUD（転倒ゲージ、操作ボタン）
- ✅ リザルト画面
...
- ❌ Redux: ボイラープレート過剰
- ❌ **cannon-es (物理エンジン)**: カスタム実装で十分（~100行で重力・衝突・転倒を実現）
...
- **Physics Module** (`src/physics/`): カスタム物理エンジン
- **Systems Module** (`src/systems/`): タップトラッカー、AI
- **Game Actors** (`src/components/game/`): 力士、土俵コンポーネント
- **UI Module** (`src/components/ui/`): HUD、コントロール
...
│   ├── state/           # 状態管理
│   │   └── gameStore.ts # Zustand グローバルステート
│   ├── physics/         # 物理エンジン
...
- 俯瞰斜め視点で土俵全体を把握しながら、連打で相手を押し出す爽快バトル。
- 8bitカラーとドット絵風フォントで懐かしいゲーム感を演出。
- カスタム物理エンジンによる転倒・押し出しのリアルな挙動。
...
- 画面中央に「トン！」ボタン配置。
  - タップ/クリックで押す力を発生
  - 連打すればするほど強い力で押せる
  - タップレート（1秒あたりの回数）を測定して力の強さを決定
- 入力フィードバック: ボタン点灯/縮みアニメ、バイブレーション（対応端末のみ）。
- 物理シミュレーション: 重力、減衰、衝突、回転による転倒判定。
...
- カスタム物理エンジン（~100行）実装
  ...
  - 減衰: 速度・角速度の自然減少
  - 衝突: 土俵との境界判定
  - 転倒: 回転角度（rotation.x）による判定
- 勝利条件: 転倒（tipping >= 1.0）OR 土俵外（distance > 4.5）
...
- **レトロスタイル**: CSS + M PLUS 1フォント（バンドル増加なし）
- **物理エンジン**: カスタム実装（重力、減衰、衝突、転倒判定）
...
**M2: 完成度向上＋番付システム**
- ✅ 物理パラメータ調整（GRAVITY, DAMPING, TAP_FORCE等）
- ✅ AI挙動改善
- ✅ 完全なUI（転倒ゲージ、連打ボタン）
- ✅ リザルト画面（転倒率表示）
- ✅ 番付システム（前頭→小結→関脇→大関→横綱）
